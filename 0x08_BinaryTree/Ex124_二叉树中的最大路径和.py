...
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        def maxGain(node) -> int:
            ...
            left_gain = maxGain(node.left)
            right_gain = maxGain(node.right)
            gain = max(node.val, node.val + left_gain, node.val + right_gain)

            path_max = max(node.val, node.val + left_gain, node.val + right_gain, node.val + left_gain + right_gain)
            if path_max > maxSum:
                maxSum = path_max
...
# 思路总结
# 考虑到这是一个二叉树，先设置一个 maxGain 函数，以某个节点为根节点的子树中，该节点必定只能选左边一路或者右边一路，将大的那个作为该节点的“贡献值”。并使用 maxSum 来维护最大的“贡献值”即可。
...
if __name__ == '__main__':
    ...
    # 调用方法并打印结果
    result = sol.maxPathSum(test_root)
    print(f"输出结果: {result}")